<template>
  <div>
    ...
    >
      <p class="text-center">{{ form.id ? 'Edit' : 'Add New' }} Vendor</p>
...
export default {
  ...
  computed: {
    ...
    validation() {
      return this.$store.state.vendor.validation
    },
    error() {
      return this.$store.state.vendor.error
    },
  },
  mounted() {
    this.loadingContainer(async () => {
      await this.$store.dispatch('vendor/loadList')
    })
  },
  methods: {
    handleEdit(v) {
    ...
    clearForm() {
      this.$store.dispatch('vendor/clearError')
      this.form = {
        id: null,
        name: null,
        address: null,
    ...
    handleDelete(id) {
      const isConfirmed = confirm(
        'Delete vendor will remove other data that correlated to this vendor, Are you sure?'
      )
      if (!isConfirmed) return
      this.loadingContainer(async () => {
        await this.$store.dispatch('vendor/delete', id)
      })
    },
    handleSave() {
      this.loadingContainer(async () => {
        if (this.form.id) await this.$store.dispatch('vendor/update', this.form)
        else await this.$store.dispatch('vendor/create', this.form)
