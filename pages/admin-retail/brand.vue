<template>
  <div>
    ...
    >
      <p class="text-center">{{ form.id ? 'Edit' : 'Add New' }} Brand</p>
...
export default {
  ...
  computed: {
    ...
    validation() {
      return this.$store.state.brand.validation
    },
    error() {
      return this.$store.state.brand.error
    },
  },
  created() {
  ...
  methods: {
    ...
    clearForm() {
      this.$store.dispatch('brand/clearError')
      this.form = {
        id: null,
        name: null,
        address: null,
